...
class ebookRead
	...
	function processEpub()
		...
		$mime = $this->readEpubFile($_SESSION['efile'], "mimetype");
		...
		$contents = $this->readEpubFile($_SESSION['efile'], "META-INF/container.xml");
		...
		$this->ebookData->toc = $this->fildFileByExt($_SESSION['efile'], "ncx");
		$this->ebookData->xpgt = $this->fildFileByExt($_SESSION['efile'], "xpgt");
		$this->ebookData->css = $this->fildFileByExt($_SESSION['efile'], "css");
		$this->loadReqOPF();
...
//		$this->removeSimpleXML();
	 ...
	 * */
	private
		function fildFileByExt($input, $ext)
		{
		$contents = null;
		if (is_file($input))
			{
			...
			if ($ret === true)
				...
				for ($x = 0; $x < $zip->numFiles; $x++)
					{
					$fileExt = basename($zip->getNameIndex($x));
					if ($fileExt == $ext)
						{
						if (!is_array($contents))
							...
							if (!isset($contents))
								{
								$contents = $zip->getNameIndex($x);
								}
							else
								{
								$placeHolder = $contents;
								$contents = array ($placeHolder);
								array_push($contents, $zip->getNameIndex($x));
								}
							}
						else
							{
	...
	private
		...
		$contents = $this->readEpubFile($_SESSION['efile'], $this->ebookData->opfPath);
		if (!isset($contents))
...
//			trigger_error("can't find the dublin core language data inside the opf file", E_USER_ERROR);
		...
		$this->ebookData->identifierId = $this->optAttributeExist($this->ebookData->identifier, "id");
...
//		$this->ebookData->titleXmlLang = (string)$this->optAttributeExist($this->ebookData->title, "xmllang");
		...
		$this->ebookData->identifierScheme = $this->optAttributeExist($this->ebookData->identifier, "opfscheme");
		$this->ebookData->identifierXsiType = $this->optAttributeExist($this->ebookData->identifier, "xsitype");
...
//		$this->ebookData->contributorOpfFileAs = (string)$this->optAttributeExist($this->ebookData->contributor, "opffile-as");
		...
		$this->ebookData->subjectXmlLang = $this->optAttributeExist($this->ebookData->subject, "xmllang");
...
//		$this->ebookData->rightsXmlLang = (string)$this->optAttributeExist($this->ebookData->rights, "xmllang");
	...
	private
		...
		if (isset($this->ebookData->toc))
			...
			$contents = $this->readEpubFile($_SESSION['efile'], $this->ebookData->toc);
			$xml = simplexml_load_string($contents);
	 ...
	 * */
	private
		function optAttributeExist($tag, $attribute)
		{
		if (isset($tag))
			{
			if (is_array($tag))
	 ...
	 * */
	private
		function getTag(SimpleXMLElement $input, $tag)
		{
	 ...
	 * */
	private
		function xmlFindData(SimpleXMLElement $xmlInput, $tag)
		{
		$array = array ();
	 ...
	 * */
	private
		function readEpubFile($zipFile, $fileLocation)
		{
		...
		if ($ext == "epub" && is_file($zipFile))
			...
			if ($ret === true)
				...
				for ($x = 0; $x < $zip->numFiles; $x++)
					{
					if ($zip->getNameIndex($x) == $fileLocation)
						{
						$contents = $zip->getFromIndex($x);
						break;
						}
					}
//			  while ($zip_entry = $zip->read($zip))
//				{
//			    	if(zip_entry_name($zip_entry) == $fileLocation
...
//				}
	 ...
	 */
	public
		function getDcTitle($optional = null)
		{
		switch ($optional)
			{
			case "XmlLang":
	 ...
	 */
	public
		function getDcLanguage($optional = null)
		{
		switch ($optional)
			{
			case "XsiType":
	 ...
	 */
	public
		function getDcIdentifier($optional = null)
		{
		switch ($optional)
			{
			case "Scheme":
	 ...
	 */
	public
		function getDcCreator($optional = null)
		{
		switch ($optional)
			{
			case "Role":
	 ...
	 */
	public
		function getDcContributor($optional = null)
		{
		switch ($optional)
			{
			case "Role":
	 ...
	 */
	public
		function getDcPublisher($optional = null)
		{
		switch ($optional)
			{
			case "XmlLang":
	 ...
	 */
	public
		function getDcSubject($optional = null)
		{
		switch ($optional)
			{
			case "XmlLang":
	 ...
	 */
	public
		function getDcDescription($optional = null)
		{
		switch ($optional)
			{
			case "XmlLang":
	 ...
	 */
	public
		function getDcDate($optional = null)
		{
		switch ($optional)
			{
			case "Event":
	 ...
	 */
	public
		function getDcType($optional = null)
		{
		switch ($optional)
			{
			case "XsiType":
	 ...
	 */
	public
		function getDcFormat($optional = null)
		{
		switch ($optional)
			{
			case "XsiType":
	 ...
	 */
	public
		function getDcSource($optional = null)
		{
		switch ($optional)
			{
			case "XmlLang":
	 ...
	 */
	public
		function getDcRelation($optional = null)
		{
		switch ($optional)
			{
			case "XmlLang":
	 ...
	 */
	public
		function getDcCoverage($optional = null)
		{
		switch ($optional)
			{
			case "XmlLang":
	 ...
	 */
	public
		function getDcRights($optional = null)
		{
		switch ($optional)
			{
			case "XmlLang":
	 ...
	 * */
	public
		function getManifestItem($index, $item)
		{
		if ($item == 'media-type')
			$item = 'type';
		if (isset($this->ebookData->manifestData[$index]->$item))
	 ...
	 * */
	public
		function getGuideItem($index, $item)
		{
	...
	public
		...
		else if (isset($_SESSION['efile']))
			{
			return $this->fildFileByExt($_SESSION['efile'], $ext);
			}
		else
			{
	 ...
	 * */
	public
		function getContentFile($location)
		{
		if (isset($this->ebookData->tempDir))
			{
			...
			{
			return $this->readEpubFile($_SESSION['efile'], $this->ebookData->contentFolder . $location);
			}
		else
			{
	 ...
	 */
	public
		function getContentById($item)
		{
		foreach ($this->ebookData->manifestData as $man)
			{
			if ($man->id == XMLstring($item))
				{
